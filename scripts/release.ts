...
async function runCommand(cmd: string, args: string[], cwd: string): Promise<{ success: boolean; output: string }> {
...
async function getLastReleaseTag(): Promise<string | null> {
  const result = await runCommand('git', ['describe', '--tags', '--abbrev=0'], PROJECT_ROOT)
  if (result.success) {
...
async function hasPackageChanges(pkgName: string, sinceTag: string | null): Promise<boolean> {
  ...
  const result = await runCommand('git', ['diff', '--name-only', sinceTag, 'HEAD', '--', pkgPath], PROJECT_ROOT)
...
async function isVersionPublished(pkgName: string, version: string): Promise<boolean> {
  const result = await runCommand('npm', ['view', `${pkgName}@${version}`, 'version'], PROJECT_ROOT)
  return result.success && result.output.trim() === version
}

async function publishPackage(pkgName: string, dryRun: boolean): Promise<boolean> {
  ...
  if (alreadyPublished) {
    console.log(`${c.gray}○ Skipping ${pkg.name}@${pkg.version} (already published)${c.reset}`)
    return true
  }
  ...
  const result = await runCommand('npm', ['publish', '--access', 'public'], pkgDir)
...
async function bumpPackageVersion(pkgName: string, type: BumpType): Promise<boolean> {
  ...
  console.log(`${c.cyan}${pkg.name}${c.reset} → ${c.green}${pkg.version}${c.reset}`)
  return true
}
...
async function rollbackVersions() {
  ...
  for (const backup of versionBackups) {
    try {
      ...
      console.log(`${c.gray}  Restored ${backup.pkgName} → ${backup.originalVersion}${c.reset}`)
    } catch {
...
async function createReleaseTag(version: string, dryRun: boolean): Promise<boolean> {
  const tag = `v${version}`
  ...
  const pushResult = await runCommand('git', ['push', 'origin', tag], PROJECT_ROOT)
...
async function release() {
  ...
  const bumpType = args.find(arg => ['patch', 'minor', 'major'].includes(arg)) as BumpType | undefined
  ...
  if (lastTag) {
    console.log(`${c.gray}Last release: ${lastTag}${c.reset}\n`)
  } else {
    console.log(`${c.gray}No previous release tags found${c.reset}\n`)
  }
  
  const packagesToPublish: string[] = []
  
  if (bumpType) {
    console.log(`${c.yellow}Checking for changes (${bumpType}):${c.reset}\n`)
    
    for (const pkgName of PUBLISH_ORDER) {
      const hasChanges = await hasPackageChanges(pkgName, lastTag)
      ...
      if (hasChanges) {
        ...
        if (existsSync(pkgJsonPath)) {
          const pkg = readPackageJson(pkgJsonPath)
          console.log(`${c.gray}○ ${pkg.name} (no changes)${c.reset}`)
        }
      }
    }
    
    if (packagesToPublish.length === 0) {
      console.log(`\n${c.yellow}No packages have changes to publish.${c.reset}\n`)
      return
    }
  ...
  if (allSuccess) {
    ...
    await rollbackVersions()
    console.log(`\n${c.brightRed}✗ Release failed - all changes rolled back${c.reset}\n`)
    process.exit(1)
  }
}
